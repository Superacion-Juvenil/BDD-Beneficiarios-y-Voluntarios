...
function getInitialSection() {
  return sessionStorage.getItem('sj_admin_section') || 'usuarios';
}
...
function UsuariosSection({ users, loading, error, stats }) {
...
function AgregarSection() {
  const navigate = useNavigate();
  useEffect(() => { navigate('/admin/nuevo'); }, [navigate]);
  return null;
}

function EvaluacionesAdminSection() {
  return (
    <div className="sj-fade" style={{
      ...
      <p style={{ margin: 0, fontSize: '0.95rem' }}>Resumen de evaluaciones próximamente.</p>
    </div>
  );
}
...
function EventosSection() {
  return (
    <div className="sj-fade" style={{
      ...
      <div style={{ fontSize: '2.5rem', marginBottom: '12px' }}>🎯</div>
      <p style={{ margin: 0, fontSize: '0.95rem' }}>Gestión de eventos próximamente.</p>
    </div>
  );
}
...
export function AdminPanel() {
  ...
  const badges = {
    usuarios: stats.total || 0,
  };

  function handleSectionChange(section) {
    setActiveSection(section);
    sessionStorage.setItem('sj_admin_section', section);
  }

  return (
    <Layout activeSection={activeSection} onSectionChange={handleSectionChange} badges={badges}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '24px 16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
          <h1 style={{ margin: 0, fontSize: '1.4rem', color: '#111827' }}>
            {activeSection === 'usuarios'     && 'Participantes'}
            {activeSection === 'agregar'      && 'Nuevo participante'}
            {activeSection === 'evaluaciones' && 'Evaluaciones'}
            {activeSection === 'eventos'      && 'Eventos'}
          </h1>
          {activeSection === 'usuarios' && (
            <Button onClick={() => handleSectionChange('agregar')}>+ Nuevo participante</Button>
          )}
        </div>

        {error && <Alert type="error" style={{ marginBottom: '16px' }}>{error}</Alert>}

        {activeSection === 'usuarios'     && <UsuariosSection users={users} loading={loading} error={error} stats={stats} />}
        {activeSection === 'agregar'      && <AgregarSection />}
        {activeSection === 'evaluaciones' && <EvaluacionesAdminSection />}
